...
void solve() {
    ...
    vector<vector<array<int,2>>> segments(m+1);
    int res = 1;
    
    for (int i = 0;i < n;i++){
        int l,r,p,q;cin >> l >> r >> p >> q;
        segments[l].push_back({r,mod_div(p,q)});
        res = mod_mul(res ,mod_sub(1,mod_div(p,q)));
    }
    ...
    function<int(int)> recur = [&](int idx) -> int {
        ...
        int res = 0;
        for (auto [r, p] : segments[idx]) {
            auto temp = recur(r+1);
            res = mod_add(res,mod_mul(temp,mod_div(p,mod_sub(1,p))));
        }

        return dp[idx] = res;
    };
    
    cout << mod_mul(res,recur(1)) << endl;
}
...
// array<int,2> dp[m+1];
    ...
    // function<array<int,2>(int)> recur = [&](int idx) -> array<int,2> {
    //     if (idx == m + 1) {
    //         return {1, 1};
    //     }
    ...
    //         auto temp = recur(r+1);
    //         if(temp[1]){
    //             res = mod_add(res,mod_mul(temp[1],mod_div(p,mod_sub(1,p))));
    //         }
    //     }

    //     return dp[idx] = {min(res,1ll), res};
    // };
