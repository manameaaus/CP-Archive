...
void solve() {
    ...
    for(int i = 1;i <= n;i++) cin >> dishes[i];
    ...
    function<int(int,int,int)> recur = [&](int curr,int prev,int rem) -> int{
        if (rem == 0){
            return 0;
        }
        if (curr > n){
        ...
        if (dp[curr][prev][rem] != -1) return dp[curr][prev][rem];

        int take = recur(curr+1,curr,rem-1) + dishes[curr] + cost[prev][curr];
        int notTake = recur(curr+1,prev,rem);


        if (curr == 2 and prev == 1){
            cout << recur(curr+1,curr,rem-1) << " " <<  dishes[curr] << " " << cost[prev][curr] << endl;
        }

        return dp[curr][prev][rem] = max(take,notTake);
    };
