...
void solve() {
    ...
    while (end < n){
        if (mp[arr[end]] == 0 and distinct == k){
            while (mp[arr[st]] != 1){
                mp[arr[st]]--;
                st++;
            }
            ...
            if (lene > r){
                ...
                if (lene < l || distinct < k){
                    if (mp[arr[end]] == 0) distinct++;
                    mp[arr[end]]++;
                    if (distinct == k && lene >= l) {
                        while (mp[arr[st]] != 1 && end-st >= l){
                            mp[arr[st]]--;
                            st++;
                            extra++;
                        }
                        ans += min(extra,r - (end-st+1)) + 1;
                        // print(st,end,ans);
                    };
                    end++;
                    ...
                    ans += min(extra,r - (end-st+1)) + 1;
                    // print(st,end,ans,lene,end-st >= l);
                    // extra = 0;
