...
void solve() {
    int n;cin >> n;
    string s;cin >> s;
    function<int(int,int,int)> calc = [&](int l,int n,int c) -> int{
        char letter = char(c);
        if (n == 1){
            return s[l] != letter;
        }
        
        int right = calc(l + n/2,n/2,c+1);
        for(int i = l;i < l + n/2;i++) if (s[i] != letter) right += 1;

        int left = calc(l,n/2,c+1);
        for(int i = l + n/2;i < l + n;i++) if (s[i] != letter) left += 1;
        
        return min(right,left);
        
    };
    cout << calc(0,n,'a'+0) << endl;
