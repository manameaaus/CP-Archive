...
void solve() {
    ...
    function<bool(int,int,int)> check = [&](int l1,int l2,int s) -> bool{
        // if (mp.contains({l1,l2,s})) return mp[{l1,l2,s}];
        bool status = true;
        for(int i = 0;i < s;i++){
            if (s1[l1+i] != s2[l2+i]) {
                status = false;
        ...
        if(!status and s%2 == 0){
            status = check(l1+s/2,l2+s/2,s/2) && check(l1,l2,s/2);
            if (!status){
                status = check(l1+s/2,l2,s/2) && check(l1,l2+s/2,s/2);
            }
        }
        
        return status;
        // return mp[{l1,l2,s}] = status;
    };

    cout << (check(0,0,n) ? "YES" : "NO") << endl;
}
