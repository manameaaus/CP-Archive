...
void solve() {
    ...
    multiset<int> exist = {all(arr)};
    multiset<int> addon;

    vi ans;

    for(int i = 0;i < n; i++){
        auto small = exist.begin();
        auto small2 = addon.begin();

        if (small2 == addon.end()){
            if (arr[i] == *small) ans.pb(*small);
            else addon.insert(arr[i]+1);
        }else{
            if (*small <= *small2 && arr[i] == *small) ans.pb(*small);
            else addon.insert(arr[i]+1);
        }
        exist.erase(exist.find(arr[i]));
    }

    for(int x:ans) cout << x << " ";
    for(int x:addon) cout << x << " ";
    cout << endl;
}
