...
int nck(int n,int k){
    // int deno = 1;
    // for (int i = 0;i < k;i++){
    //     deno = mod_mul(deno,n-i);
    //     deno = mod_div(deno,i+1);
    // }
    // return deno;
    return mod_div(fact[n],mod_mul(fact[n-k],fact[k]));
}
...
void solve() {
    ...
    for(int i = 1;i <= n;i++){
        if ((i == 1) || (((i-1) * 2) > n) || ((2 * i) - (i-1) > n)){
            cout << 0 << " ";
            continue;
        }else{
            int req_arr = 2*(i-1);
            cout << mod_mul(mod_mul(n-req_arr+1,nck(n - i - 1, i-2)),mod_mul(fact[req_arr],fact[n-req_arr])) << " ";
        }
        // cout << req_arr << <<  << endl;
    }
    cout << endl;
