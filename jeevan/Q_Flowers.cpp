...
void solve() {
    ...
    for(int i = 1;i < n + 1;i++){
        ...
        if (it != mp.end()) curr += (*it)[1];
        mp.insert({-arr[i],curr});

        for (auto it = sorted.lower_bound({-curr,-1}); it != sorted.end(); ) {
            if ((*it)[1] > arr[i]) {
                mp.erase(mp.lower_bound({-(*it)[1],-(*it)[0]}));
                it = sorted.erase(it); 
            } else {
                break;
            }
        }
        sorted.insert({-curr,arr[i]});
    }
    print(-(*sorted.begin())[0]);
