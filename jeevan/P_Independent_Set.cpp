...
void solve() {
    ...
    function<int(int,int)> dfs = [&](int node,int black) -> int {
        if (dp[node][black] != -1) return dp[node][black];
        vis[node] = 1;
        int ans_w = 1,ans_b = 1 - black;
        for(auto nbr:adj[node]){
            if (!vis[nbr]) ans_w = mod_mul(ans_w,dfs(nbr,0));
        }
        if (!black){
            for(auto nbr:adj[node]){
                if (!vis[nbr]) ans_b = mod_mul(ans_b,dfs(nbr,1));
            }
        }
        vis[node] = 0;
        return dp[node][black] = mod_add(ans_w , ans_b);
    };
