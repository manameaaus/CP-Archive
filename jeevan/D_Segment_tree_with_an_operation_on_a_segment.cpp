...
int query(int node, int st, int nd, int idx) {
    if (st == nd) {
        return seg[node]+extra[node];
    }

    int mid = (st+nd)/2;
    if (idx <= mid) return query(2*node, st, mid, idx) + extra[node];
    else return query(2*node+1, mid+1, nd, idx) + extra[node];
...
void solve() {
    ...
    for (int i = 0;i < q; i++){
        ...
        if (c == 'g'){
            int l;cin >> l;
            cout << query(1,1,ref,l) << '\n';
        }else{
            int l,r,val;cin >> l >> r >> val;
            update(1,1,ref,l,r,val);
        }
    }
