...
#define print3(zzz) for (auto &zzzzz : zzz) {print2(zzzzz)}
...
void solve() {
    ...
    for (int i = 0;i < n;i++){
        for (int j = i-1;j > -1;j--){
            for (int k = i;k > j;k--){
                dp[i][j][0] = min(dp[i][j][0],(dp[k-1][j][0] * x + dp[i][k][0] * y + z) % 50);
                dp[i][j][1] = min(dp[i][j][1],dp[k-1][j][1] + (dp[k-1][j][0] * dp[i][k][0]) + dp[i][k][1]);
            }
        }
    }
