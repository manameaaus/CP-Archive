...
void solve() {
    int n,m; cin >> n >> m;
    vector<int> sss(n); for (auto &e : sss) {cin >> e;}
    vector<int> monster(m); for (auto &e : monster) {cin >> e;}
    vector<int> repeat(m); for (auto &e : repeat) {cin >> e;}

    multiset<pair<int,int>> nzero;
    multiset<int> use,zero,run;
    ...
    for (int i = 0; i < m; ++i) {
        if (repeat[i] == 0) {
            zero.insert(-monster[i]);
        } else {
            nzero.insert({monster[i],repeat[i]});
        }
    }
    ...
    while(it != use.end()) {
        int cur = (*it);
        while(!nzero.empty() && (*(nzero.begin())).first <= cur) {
            run.insert(-(*nzero.begin()).second);
            ans++;
            nzero.erase(nzero.begin());
        }
        if (!run.empty()) {
            auto it2 = run.begin();
            if (-(*it2) > cur) {
